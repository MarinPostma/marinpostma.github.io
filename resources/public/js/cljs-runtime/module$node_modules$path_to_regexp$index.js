shadow$provide.module$node_modules$path_to_regexp$index=function(global,process,require,module,exports,shadow$shims){function parse(str,options){var tokens=[],key=0,index=0,path="";options=options&&options.delimiter||"/";for(var res;null!=(res=PATH_REGEXP.exec(str));){var m=res[0],escaped=res[1],offset=res.index;path+=str.slice(index,offset);index=offset+m.length;if(escaped)path+=escaped[1];else{var next=str[index];m=res[2];escaped=res[3];var capture=res[4],group=res[5],modifier=res[6];offset=res[7];
path&&(tokens.push(path),path="");next=null!=m&&null!=next&&next!==m;var repeat="+"===modifier||"*"===modifier;modifier="?"===modifier||"*"===modifier;res=res[2]||options;capture=capture||group;tokens.push({name:escaped||key++,prefix:m||"",delimiter:res,optional:modifier,repeat:repeat,partial:next,asterisk:!!offset,pattern:capture?capture.replace(/([=!:$\/()])/g,"\\$1"):offset?".*":"[^"+escapeString(res)+"]+?"})}}index<str.length&&(path+=str.substr(index));path&&tokens.push(path);return tokens}function encodeURIComponentPretty(str){return encodeURI(str).replace(/[\/?#]/g,
